...
class CFKeyProvider {
    ...
    async getCfTargetFromConfigFile() {
        ...
        try {
            ...
            this.logger.debug(`getCfTargetFromConfigFile: ${err}`);
        }
    }
    ...
    async getCfSpaceInfo() {
        ...
        const orgs = await this._req(targetWithAuth, `/v3/organizations?names=${encodeURIComponent(encodeURIComponent(org))}`);
        if (!(orgs && orgs.resources.length === 1)) {
            throw new Error(`CF org ${org} not found!`);
        }

        const orgGuid = orgs.resources[0].guid;
        const spaces = await this._req(targetWithAuth, `/v3/spaces?names=${encodeURIComponent(encodeURIComponent(space))}&organization_guids=${encodeURIComponent(encodeURIComponent(orgGuid))}`)
...
class BatchRequest {
    ...
    async executeRequest() {
        ...
        const params = Object.keys(filterParams).map(
            name => `${name}=${encodeURIComponent(filterParams[name].map(value => encodeURIComponent(value)).join(','))}`
        ).join("&");
