  ...
  try {
    ...
    try { 
      ...
      for(let file of flist) {
        const entity = path.basename(file, '.csv').replace('-', '.')
        csvs[entity] = await read(path.join(root, 'data', file)) 
      }
    }
    catch(e) { if (e.code !== 'ENOENT') throw e }

    return { extCsn: extCsn && JSON.parse(extCsn), bundles, csvs }
  } finally {
    rimraf (root)
  }
}
